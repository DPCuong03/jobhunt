...
const fetchFaq = async (id: string): Promise<FaqData> => {
  const res = await api.get(`/admin/faqs/${id}`);
  return res.data;
};

const createFaq = async (data: Omit<FaqData, "id">): Promise<FaqData> => {
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-[#f0ede8] bg-[#fafaf9] flex-wrap">
        ...
        {TOOLBAR_ACTIONS.map(({ cmd, label, style }) => (
          <button
            ...
            type="button"
            onClick={() => exec(cmd)}
            className={`w-7 h-7 flex items-center justify-center rounded text-gray-600 hover:bg-[#e5e2db] transition-colors text-[13px] ${style}`}
            title={cmd}
          >
            {label}
          </button>
...
function EditFaqForm({ faqId }: EditFaqFormProps) {
  ...
  // Fetch existing FAQ for edit mode
  const { isLoading: isFetching, error: fetchError } = useQuery({
    queryKey: ["adminFaq", faqId],
    queryFn: () => fetchFaq(faqId!),
    enabled: isEdit,
    staleTime: 0,
    // @ts-ignore - react-query v5 onSuccess is via select/useEffect
  });
  ...
  const createMutation = useMutation({
    ...
    onError: () => setFormError("Failed to create FAQ. Please try again."),
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["adminFaqs"] });
      queryClient.invalidateQueries({ queryKey: ["adminFaq", faqId] });
      setSuccess(true);
    ...
    onError: (error: any) => {
      toast.error(error.message || "Failed to update FAQ. Please try again.");
    },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEdit && faqId) {
      updateMutation.mutate({ id: faqId, data: { question, answer } });
    } else {
  ...
  if (isEdit && fetchError) {
    return (
      <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl p-5 text-red-700">
        ...
        <p className="text-sm font-medium">Failed to load FAQ data.</p>
      </div>
    );
  }
