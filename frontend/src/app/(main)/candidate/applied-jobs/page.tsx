...
import { useQuery, keepPreviousData } from "@tanstack/react-query"; // Thêm keepPreviousData

import { Eye, Loader2, AlertCircle } from "lucide-react";

import Link from "next/link"; // Dùng Link thay vì <a>
...
const fetchAppliedJobs = async () => {
  try {
    ...
    const message =
      error.response?.data?.message || "Could not fetch applied jobs";
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
      ...
      <div className="p-5 border-b border-gray-100 bg-white flex justify-between items-center">
        <h1 className="text-xl font-bold text-gray-800">Applied Jobs</h1>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50/50">
              <th className="p-4 w-20 text-center font-bold text-sm text-gray-700">
                ID
              </th>

              <th className="p-4 w-1/4 font-bold text-sm text-gray-700">
                Job Title
              </th>

              <th className="p-4 font-bold text-sm text-gray-700">Company</th>

              <th className="p-4 w-32 font-bold text-sm text-gray-700 text-center">
                Status
              </th>
              <th className="p-4 w-40 font-bold text-sm text-gray-700 text-center">
                Cover Letter
              </th>
              <th className="p-4 w-24 font-bold text-sm text-gray-700 text-center">
                Detail
              </th>
            </tr>
          </thead>
              ...
              applications.map((app: any, index: number) => (
                ...
                >
                  ...
                  <td className="p-4 text-sm">
                    <span className="inline-flex items-center px-3 py-1 rounded-md text-[10px] font-black uppercase bg-blue-700 text-white shadow-sm">
                      {app.status}
                    </span>
                  </td>

                  <td className="p-4 text-center">
                    <button className="px-4 py-1.5 bg-[#FFC107] hover:bg-[#e0a800] text-black text-xs  rounded shadow-sm transition-transform active:scale-95">
                      Cover Letter
                    </button>
                  </td>
