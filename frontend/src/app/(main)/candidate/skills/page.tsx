...
const fetchSkills = async () => {
  try {
    ...
    const message =
      error.response?.data?.message || "Could not fetch skills records";
...
const deleteSkill = async (id: number) => {
  try {
    const response = await api.delete(`/candidate/skills/delete/${id}`);
    return response.data;
  } catch (error: any) {
    const message =
      error.response?.data?.message || "Could not delete skill record";
    throw new Error(message);
  }
};
              ...
              <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="p-4">
                  <div className="flex justify-center gap-2">
                    <Link
                      href={`/candidate/skills/edit/${item.id}`}
